...
var get_variable = req.body.player_choice;
var query1 = 'SELECT id, name FROM football_players;';
var query2 = 'SELECT name, year, major, passing_yards, rushing_yards, receiving_yards FROM football_players WHERE id =' +  get_variable + ';';
var query3 = 'SELECT COUNT(*) FROM football_players AS p INNER JOIN football_games AS g ON p.id = ANY(g.players) WHERE p.id=(SELECT id FROM football_players WHERE id=' + get_variable + ');';
db.task('get-everything', task => {
    return task.batch([
        task.any(query1),
        task.any(query2),
