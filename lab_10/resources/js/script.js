...
$(document).ready(function() {
  ...
  $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
    ...
    */
    var image_today = data.current.weather_icons[0];
    ...
    document.getElementById("temp_today").innerText = temp_today + " F ";

    var percentage = 0;//temp_today;
    ...
    if(temp_today > 85)
    {
     ...
     percentage = 0;
      ...
      percentage = temp_today/100*85;
    }
    $("#thermometer_inner").height(percentage+"%"); 
    console.log(document.getElementById("thermometer_inner").style.height);

    var precip_today = data.current.precip;
    document.getElementById("precip_today").innerText = precip_today + " m";

    var humidity_today = data.current.humidity;
    document.getElementById("humidity_today").innerText = precip_today + "%";

    var wind_today = Math.round(data.current.wind_speed/1.609344);
    document.getElementById("wind_today").innerText = wind_today;

    var summary_today = data.current.weather_descriptions;
    document.getElementById("summary_today").innerText = summary_today;
    ...
    /*
      ...
      <Hint1 - To access the forecast data> You need to make sure to carefully see the
      JSON response to see how to access the forecast data. While creating the key to
      access forecast data make sure to convert it into a string using the toString() method.

      <Hint2 - To add the cards to the HTML> -
      Make sure to use string concatenation to add the html code for the daily weather cards.
      This should be set to the innerHTML for the 5_day_forecast.
    */
    ...
    for (i = 0; i < 5; i++)
    {
      var forecast_data = getKey(i);
      forecast_string += '<div style="width:20%;">' +
            '<div class="card">'+
              '<div class="card-body">' +
                '<h5 class="card-title">' + dayOfWeek + '</h5>'+
                '<p class="card-text">High: ' + getFarenheitTemp(forecast_data.maxtemp) + ' F<br>' +
                  'Low: ' + getFarenheitTemp(forecast_data.mintemp) + ' F<br>' +
                  'Sunrise: ' + forecast_data.astro.sunrise +'<br>' +
                  'Sunset: ' + forecast_data.astro.sunset + '</p>' +
              '</div>' +
            '</div>' +
          '</div>' 
    }
    document.getElementById("5_day_forecast").innerHTML = forecast_string;
  })
});
